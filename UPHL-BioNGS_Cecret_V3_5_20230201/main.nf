...
if (params.config_file) {
  def src = new File("${workflow.launchDir}/configs/cecret_config_template.config")
...
//# outdir params
params.outdir                               = workflow.launchDir + '/out/cecret'
...
if (params.species        == 'sarscov2' ) {
  params.reference_genome                   = workflow.launchDir + '/configs/MN908947.3.fasta'
  params.gff                                = workflow.launchDir + '/configs/MN908947.3.gff'
  println("Using the subworkflow for SARS-CoV-2")
} else if (params.species == 'mpx') {
  params.reference_genome                   = workflow.launchDir + '/configs/NC_063383.1.fasta'
  params.gff                                = workflow.launchDir + '/configs/NC_063383.1.gff3'
...
if ( params.primer_set        == 'ncov_V3' ) {
  params.primer_bed                         = workflow.launchDir + '/configs/artic_V3_nCoV-2019.primer.bed'
  params.amplicon_bed                       = workflow.launchDir + '/configs/artic_V3_nCoV-2019.insert.bed'
} else if ( params.primer_set == 'ncov_V4' ) {
  params.primer_bed                         = workflow.launchDir + '/configs/artic_V4_SARS-CoV-2.primer.bed'
  params.amplicon_bed                       = workflow.launchDir + '/configs/artic_V4_SARS-CoV-2.insert.bed'
} else if ( params.primer_set == 'ncov_V4.1' ) {
  params.primer_bed                         = workflow.launchDir + '/configs/artic_V4.1_SARS-CoV-2.primer.bed'
  params.amplicon_bed                       = workflow.launchDir + '/configs/artic_V4.1_SARS-CoV-2.insert.bed'
} else if ( params.primer_set == 'mpx_idt' ) {
  params.primer_bed                         = workflow.launchDir + '/configs/mpx_idt_primer.bed'
  params.amplicon_bed                       = workflow.launchDir + '/configs/mpx_idt_insert.bed'
} else if ( params.primer_set == 'mpx_primalseq' ) {
  params.primer_bed                         = workflow.launchDir + '/configs/mpx_primalseq_primer.bed'
  params.amplicon_bed                       = workflow.launchDir + '/configs/mpx_primalseq_insert.bed'
} else {
...
//# getting scripts
ch_combine_results_script = Channel.fromPath("${workflow.launchDir}/bin/combine_results.py", type:'file')
